---
up: []
related: ["[[themes/ai-infrastructure]]", "[[strategies/options/bull-call-spread]]", "[[strategies/options/risk-reversal]]", "[[concepts/momentum]]", "[[concepts/fcf-yield]]"]
created: 2026-04-10
type: "ticker"
sector: "Technology"
industry: "Social Media / Advertising"
tags: [ticker, mega-cap, advertising, ai]
---

# META — Meta Platforms

**As of 2026-04-10 | Regime: transitioning**
Decision: Buy META Jun 2026 620/700 call spread + sell Jun 2026 540 put (risk reversal)

## 2026-04-10
- Bull: Core ads high-growth/high-margin with cash-backed earnings; CFO > NI in multiple quarters; back above key EMAs with momentum improving toward ~$631 breakout
- Bear: EU DMA monetization hit + fixed-cost AI commitments
- Bull stronger on operating fundamentals; defined-risk preferred in fragile macro
- Catalyst: Break above $631 + positive earnings read-through on ad demand and Reels/WhatsApp AI monetization
- Timeframe: 6-10 weeks (June 2026 expiry)
- Key risk: EU DMA/DSA enforcement; capex guide-down in FCF; macro risk-off. Cut if META < $592-597
- Source: [[sources/brief-2026-04-10]]

## 2026-04-10 (manual brief update)
- META at ~$628, approaching $631 breakout resistance
- **CoreWeave signed $21B NEW deal with Meta → total $35B through 2032** (2026-04-09, Bloomberg/CNBC). New capacity uses NVIDIA Rubin chips. CoreWeave also proposed $3B convertible notes offering.
- Bull case strengthened: $35B commitment confirms Meta is most aggressive hyperscaler in AI capex. Buying capacity (CoreWeave) rather than building in-house → faster AI training ramp.
- Bear unchanged: EU DMA/DSA enforcement; $21B is operating EXPENSE commitment (hits P&L), not capex
- **Action:** Hold risk reversal (620/700 call + sold 540 put). Watch $631 breakout — if cleared, captures upside.
- See also: [[themes/ai-infrastructure-financing-risk]] — CoreWeave systemic risk
- Source: [[sources/brief-2026-04-10-manual]]

## 2026-04-11 (manual brief)
- Spot $629.86 (+0.23%). Recovery from $520.26 low (3/27) to $629.86 (+21%). CoreWeave deal spike on 4/8 ($592->$629) consolidating. RV 30d: 52.8%.
- $21B expanded CoreWeave deal (total $35B through 2032). GB300 + Vera Rubin infrastructure. Diversifying from AWS/Azure.
- Insiders: MSPR -1.0 | 19 sells | $3.96M sold | Zero buys (low $ value relative to market cap)
- Market reacted -2% to the $21B deal announcement — skeptical of spend magnitude
- **Decision: Pass, wait for Q1 earnings.** $35B CoreWeave = massive capex burden. Unclear when ROI materializes.
- Source: [[sources/brief-2026-04-11-manual]]

## 2026-04-11 (automated pipeline)
- Pipeline debate: bull case stronger. FY2025 revenue $200.97B (+22%), NI $60.46B, net margin 30.1%, GM 82%, OpM 41.3%.
- FCF $23.43B. EV/EBITDA 15.7. ROE 30.2%. DAP 3.58B (Dec 2025). Short interest 1.1%.
- Fresh bullish 8/21 EMA cross (1 day ago). RSI 57.1. OBV rising. ATM IV 44.1% < RV 52.8% — options underpriced.
- **Pipeline trade:** Buy META 2-3 month call spread (ATM / +5-7% OTM). Stop if < $622. Target ~1.5x return on premium.
- Bear: $21B CoreWeave = multi-year cost commitment, not monetization catalyst. Piotroski 5/9. D/E 39.16 (net cash negative). Insider selling pattern (COO Olivan weekly sales).
- Source: [[sources/brief-2026-04-11]]

**See also:** [[bull-call-spread]] | [[risk-reversal]] | [[momentum]] | [[fcf-yield]]

> [!quote]- Sources
> - [[sources/brief-2026-04-10]] — initial observation
> - [[sources/brief-2026-04-10-manual]] — CoreWeave $35B deal, deepened analysis
> - [[sources/brief-2026-04-11-manual]] — pass, wait Q1 earnings
> - [[sources/brief-2026-04-11]] — pipeline call spread trade, full debate
