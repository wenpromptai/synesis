...
class SECEdgarClient:
    ...
    # ─────────────────────────────────────────────────────────────
    # 13F-HR Holdings (Hedge Fund Monitoring)
    # ─────────────────────────────────────────────────────────────
    ...
    async def get_13f_filings(self, cik: str, limit: int = 2) -> list[SECFiling]:
        ...
        cache_key = f"{CACHE_PREFIX}:13f_filings:{cik}"
        ...
        try:
            resp = await self._fetch(f"{SEC_BASE_URL}/submissions/CIK{padded_cik}.json")
            resp.raise_for_status()
            data: dict[str, Any] = orjson.loads(resp.content)
        except Exception as e:
        ...
        for i in range(n):
            ...
            filings.append(
                SECFiling(
                    ...
                    accession_number=accession_numbers[i],
                    primary_document=doc,
                    items=report_dates[i] if i < len(report_dates) else "",
                    url=url,
                )
            )
    ...
    async def get_13f_holdings(self, filing: SECFiling, cik: str) -> Filing13F | None:
        ...
        """
        from synesis.core.constants import SEC_13F_HOLDINGS_CACHE_TTL
        ...
        try:
            resp = await self._fetch(index_url)
            resp.raise_for_status()
        ...
        # Find the InfoTable document — naming varies by filer:
        # "infotable.xml", "form13f_*.xml", "renaissance13F*_holding.xml", "50240.xml", etc.
        # Strategy: pick the XML file that isn't primary_doc.xml or index-related
        ...
        for item in directory.get("item", []):
            ...
            if "-index" in name_lower:
                continue
            # This should be the InfoTable XML
            infotable_name = name
            break
        ...
        try:
            resp = await self._fetch(xml_url)
            resp.raise_for_status()
        ...
        # Parse report date from filing.items (we stored reportDate there)
        try:
            report_date = date.fromisoformat(filing.items) if filing.items else filing.filed_date
        except ValueError:
            report_date = filing.filed_date
    ...
    async def compare_13f_quarters(self, cik: str, fund_name: str) -> dict[str, Any] | None:
        ...
        """
        from synesis.core.constants import SEC_13F_DIFF_CACHE_TTL

        cache_key = f"{CACHE_PREFIX}:13f_diff:{cik}"
        cached = await self._redis.get(cache_key)
        if cached:
            try:
        ...
        await self._redis.set(cache_key, orjson.dumps(result), ex=SEC_13F_DIFF_CACHE_TTL)
        return result
