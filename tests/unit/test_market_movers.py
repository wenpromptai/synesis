...
class TestMarketMoversJob:
    ...
    async def test_job_saves_to_diary(self) -> None:
        """Job persists data to diary with source='market_movers'."""
        from synesis.processing.market.job import market_movers_job
        ...
        with (
            ...
            mock_settings.return_value.discord_brief_webhook_url = MagicMock()
            mock_settings.return_value.discord_webhook_url = None
            await market_movers_job(mock_redis, db=mock_db)

        mock_db.upsert_diary_entry.assert_called_once()
        call_kwargs = mock_db.upsert_diary_entry.call_args[1]
        assert call_kwargs["source"] == "market_movers"
        assert call_kwargs["payload"]["market_data"] == data.model_dump(mode="json")
    ...
    async def test_job_diary_failure_does_not_crash(self) -> None:
        ...
        with (
            ...
            mock_settings.return_value.discord_brief_webhook_url = MagicMock()
            mock_settings.return_value.discord_webhook_url = None
            await market_movers_job(mock_redis, db=mock_db)  # Should not raise
    ...
    async def test_job_no_db_skips_diary(self) -> None:
        ...
        with (
            ...
            mock_settings.return_value.discord_brief_webhook_url = MagicMock()
            mock_settings.return_value.discord_webhook_url = None
            await market_movers_job(mock_redis)  # db=None by default, should not raise
