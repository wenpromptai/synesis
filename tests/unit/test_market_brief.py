"""Tests for the market brief pipeline (processing/market/)."""
...
from synesis.processing.market.snapshot import (
    fetch_market_brief,
...
def _make_brief(**overrides: Any) -> MarketBriefData:
    defaults: dict[str, Any] = {
        "equities": [
    ...
    defaults.update(overrides)
    return MarketBriefData(**defaults)
...
class TestMarketBriefDataModel:
    def test_round_trip(self) -> None:
        brief = _make_brief()
        dumped = brief.model_dump(mode="json")
        restored = MarketBriefData.model_validate(dumped)
    ...
    def test_empty_movers(self) -> None:
        ...
        brief = _make_brief(movers=movers)
        assert len(brief.movers.gainers) == 0

    def test_no_volatility(self) -> None:
        brief = _make_brief(volatility=None)
        assert brief.volatility is None
...
class TestFormatMarketDataForLlm:
    def test_basic_output(self) -> None:
        brief = _make_brief()
        text, sector_display = format_market_data_for_llm(brief)
        assert "## MARKET DATA" in text
        assert "SPY +1.0%" in text
        assert "QQQ +1.1%" in text
        assert "IWM -0.9%" in text
        assert "VIX 14.5 (-0.7)" in text
        assert "Tech: XLK +1.5%" in text
        assert sector_display  # non-empty

    def test_movers_section(self) -> None:
        brief = _make_brief()
        text, _ = format_market_data_for_llm(brief)
        assert "## TOP MOVERS" in text
        assert "NVDA +8.5% (Technology)" in text
        assert "META -4.3% (Communication Services)" in text
        assert "AAPL +0.5% (Technology)" in text

    def test_empty_movers_no_section(self) -> None:
        movers = MarketMovers(gainers=[], losers=[], most_actives=[], fetched_at=_NOW)
        brief = _make_brief(movers=movers)
        text, _ = format_market_data_for_llm(brief)
    ...
    def test_missing_prices(self) -> None:
        """Tickers with None change_pct are omitted."""
        brief = _make_brief(
            equities=[
                TickerChange(ticker="SPY", last=None, prev_close=None, change_pct=None),
            ]
        )
        text, _ = format_market_data_for_llm(brief)
        assert (
            "SPY" not in text.split("## TOP MOVERS")[0]
            if "## TOP MOVERS" in text
            else "SPY" not in text
        )

    def test_vix_none(self) -> None:
        brief = _make_brief(volatility=TickerChange(ticker="^VIX", last=None, prev_close=None))
        text, _ = format_market_data_for_llm(brief)
        assert "Volatility" not in text

    def test_sector_display_returned(self) -> None:
        brief = _make_brief()
        _, sector_display = format_market_data_for_llm(brief)
        assert "Tech: XLK" in sector_display
        assert "Energy: XLE" in sector_display

    def test_gainers_without_sector(self) -> None:
        movers = _make_movers(
            gainers=[
                MarketMover(ticker="ABC", change_pct=3.0, sector=None),
            ]
        )
        brief = _make_brief(movers=movers)
        text, _ = format_market_data_for_llm(brief)
        assert "ABC +3.0%" in text
        # No sector tag
        assert "()" not in text


class TestFetchMarketBrief:
    @pytest.mark.asyncio
    async def test_fetches_quotes_and_movers(self) -> None:
        """fetch_market_brief returns structured data from YFinanceClient."""
        ...
        with (
            ...
            brief = await fetch_market_brief(mock_redis)

        assert len(brief.equities) == 3
        assert len(brief.rates_fx) == 2
        assert len(brief.commodities) == 2
        assert brief.volatility is not None
        assert brief.volatility.ticker == "^VIX"
        assert brief.equities[0].name == "SPDR S&P 500"
        assert len(brief.sectors) == 11  # All sector tickers
        assert brief.movers.gainers[0].ticker == "NVDA"

    @pytest.mark.asyncio
    async def test_movers_failure_graceful(self) -> None:
        """If get_market_movers fails, brief still returns with empty movers."""
        mock_redis = AsyncMock()
        ...
        assert brief.movers.gainers == []
        assert brief.movers.losers == []
        assert brief.movers.most_actives == []
    ...
    async def test_quote_failure_graceful(self) -> None:
        ...
        spy = next(tc for tc in brief.equities if tc.ticker == "SPY")
    ...
    async def test_sectors_sorted_by_performance(self) -> None:
        ...
        sector_tickers = [s.ticker for s in brief.sectors]
...
class TestFormatMarketBriefEmbeds:
    ...
    def test_embed_structure(self) -> None:
        ...
        messages = format_market_brief_embeds(brief)
        embed = messages[0][0]
        assert "Market Brief" in embed["title"]
    ...
    def test_footer_on_last_message(self) -> None:
        ...
        assert last_embed["footer"]["text"] == "Synesis Market Brief"
    ...
    def test_volatility_no_prev_close(self) -> None:
        ...
        brief = _make_brief(volatility=TickerChange(ticker="^VIX", last=16.0, prev_close=None))
        messages = format_market_brief_embeds(brief)
        all_fields = [f for msg in messages for e in msg for f in e.get("fields", [])]
        vol_field = next(f for f in all_fields if "Volatility" in f["name"])
    ...
    def test_volatility_last_none(self) -> None:
        ...
        brief = _make_brief(volatility=TickerChange(ticker="^VIX", last=None, prev_close=15.0))
        messages = format_market_brief_embeds(brief)
        all_fields = [f for msg in messages for e in msg for f in e.get("fields", [])]
        field_names = [f["name"] for f in all_fields]
        assert not any("Volatility" in name for name in field_names)
...
class TestMarketBriefJob:
    ...
    async def test_job_sends_to_discord(self) -> None:
        ...
        assert any("Market Brief" in str(e.get("title", "")) for e in embeds)

    @pytest.mark.asyncio
    async def test_job_multi_message_sends_each(self) -> None:
        """When format_market_brief_embeds returns 2+ messages, each is sent."""
        from synesis.processing.market.job import market_brief_job
        ...
        with (
            ...
            patch(
                "synesis.processing.market.job.format_market_brief_embeds",
...
class TestEventDigestMarketIntegration:
    def test_digest_imports_from_market_module(self) -> None:
        """Verify that events/digest.py imports from processing.market.snapshot."""
        import synesis.processing.events.digest as digest_mod

        assert digest_mod.fetch_market_brief is fetch_market_brief
        assert digest_mod.format_market_data_for_llm is format_market_data_for_llm
...
class TestSchedulerRegistration:
    def test_market_brief_job_in_scheduler(self) -> None:
        """market_brief_job is exported from scheduler module."""
        from synesis.agent.scheduler import market_brief_job

        assert callable(market_brief_job)

    def test_market_brief_in_init(self) -> None:
        """market_brief_job is re-exported from processing.market."""
        from synesis.processing.market import market_brief_job

        assert callable(market_brief_job)


# ---------------------------------------------------------------------------
# API route tests for POST /market/brief
# ---------------------------------------------------------------------------


class TestMarketBriefAPI:
    @pytest.fixture()
    def _app(self):
    ...
    async def test_trigger_happy_path(self, _app, client) -> None:
        """POST /market/brief returns triggered when trigger_fn exists."""
        self._state.trigger_fns["market_brief"] = AsyncMock()
        r = await client.post("/api/v1/market/brief")
        assert r.status_code == 200
        assert r.json()["status"] == "triggered"

    @pytest.mark.asyncio
    async def test_trigger_503_when_missing(self, _app, client) -> None:
        """POST /market/brief returns 503 when trigger is not configured."""
